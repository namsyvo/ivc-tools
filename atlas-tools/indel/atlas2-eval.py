...
result_path = "/backup/qmtran/Atlas_Indel2/soybean"
...
result_file_path = "prec-rec/prec-rec-atlas-indel2-soybean-test.txt"
...
#	for seq_err in ['0.01', '0.02', '0.04']:
		...
		snp_num = len(true_indel) # capture indel only, use snp_num for all called indels
		...
		for key, value in snp.iteritems():
			if key - 1 in true_snp:
				...
				#if len(value) > 1:
				called_indel_num += 1
				if true_snp[key-1] == value[1:] or true_snp[key-1] == value:
					true_called_snp_num += 1
					if len(value) > 1:
						true_called_indel_num += 1
			if key - 2 in true_snp:
				called_snp_num += 1
				#if len(value) > 1:
				called_indel_num += 1
				if true_snp[key-2] == value[1:] or true_snp[key-2] == value:
					true_called_snp_num += 1
					if len(value) > 1:
						true_called_indel_num += 1
			if key in true_snp:
				...
				#if len(value) > 1:
				called_indel_num += 1
				if true_snp[key] == value[1:] or true_snp[key] == value:
					true_called_snp_num += 1
					if len(value) > 1:
						true_called_indel_num += 1
