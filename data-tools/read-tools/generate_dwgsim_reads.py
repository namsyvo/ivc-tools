...
for rl in read_lens:
	for err in seq_errs:
		for rn in read_nums:
			...
			'''
			cmd = "/home/SNP/genome-tools/DWGSIM/dwgsim -N " + str(rn) + " -1 " + str(rl) + " -2 " + str(rl) \
			+ " -e " + str(err) + " -E " + str(err) + " -r 0 -m " + "/data/nsvo/test_data/GRCh37_chr1/refs/dip_af_sid_mutant/dwgsim_reads_100.0.00015-0.0015.1246253.mutations.txt" + " " + ref_path + " " + result_path \
			+ "/dwgsim_reads_" + str(rl) + "." + str(err) + "." + str(rn) + " &"
			print cmd
			'''
			cmd = "/home/SNP/genome-tools/DWGSIM/dwgsim -N " + str(rn) + " -1 " + str(rl) + " -2 " + str(rl) \
