...
snp_prof = {}
for line in open(os.path.join(ref_path, snp_prof_file)):
    if line[0] != '#':
...
for p in ref_para[0:1]:
    ...
    new_snp_file = os.path.join(ref_path, "alt_mutant", "new_snps.txt")
    new_indel_file = os.path.join(ref_path, "alt_mutant", "new_indels.txt")
    ...
    for line in open(known_variant_file):
        ...
        for line in open(new_snp_file):
            info = line.strip().split()
            snp_prof_part[int(info[0])] = info[1:]

        for line in open(new_indel_file):
            info = line.strip().split()
            snp_prof_part[int(info[0])] = info[1:]


    variant_comp_file = open(os.path.join(ref_path, "variant_comp_" + str(p) + ".txt"), "w")
    variant_part_file = open(os.path.join(ref_path, "variant_part_" + str(p) + ".txt"), "w")
    ...
    for key, val in snp_prof_part.iteritems():
        variant_part_file.write(str(key) + "\t")
        for v in val:
            variant_part_file.write(str(v) + "\t")
        variant_part_file.write("\n")
        snp_prof_part_file.write(snp_prof[key] + "\n")
