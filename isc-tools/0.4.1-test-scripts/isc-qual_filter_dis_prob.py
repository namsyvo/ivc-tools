    ...
    for rl in read_lens:
        for err in seq_errs:
            for rn in read_nums:
                ...
                sorted_snp = sorted(snp.iteritems(), key=lambda x: float(x[1][0][2]), reverse=True)
                ...
                fp_snp_comp_file = open(file_prefix + ".fp_snp_comp." + str(filter_num) + "." + str(confi) + ".txt", "w")
                fp_snp_none_file = open(file_prefix + ".fp_snp_none." + str(filter_num) + "." + str(confi) + ".txt", "w")
                fp_snp_other_file = open(file_prefix + ".fp_snp_other." + str(filter_num) + "." + str(confi) + ".txt", "w")
                snp_info_file.write("rank\tpos\tsnp\tqual\tprob\tnum\taln_dis\tchr_dis\tchr_diff\taln_prob\tchr_prob\tread_id\tsnp_indel\ttp_fp\tloc_region\tfilter_region\n")
                fp_snp_comp_file.write("pos\tsnp\tqual\tprob\tnum\taln_dis\tchr_dis\tchr_diff\taln_prob\tchr_prob\tread_id\tsnp_base\tbase_num\n")
                fp_snp_none_file.write("pos\tsnp\tqual\tprob\tnum\taln_dis\tchr_dis\tchr_diff\taln_prob\tchr_prob\tread_id\tsnp_base\tbase_num\n")
                fp_snp_other_file.write("pos\tsnp\tqual\tprob\tnum\taln_dis\tchr_dis\tchr_diff\taln_prob\tchr_prob\tread_id\tsnp_base\tbase_num\n")
                rank = 0
                snp_num = 0
                for key, value_arr in rev_sorted_snp:
                    ...
                    for value in value_arr:
                        val_info = "\t".join(value[0:6]) + "\t" + str(abs(int(value[6]))) + "\t" + str(abs(int(value[7]))) + "\t" + "\t".join(value[8:11])
                        if key in true_snp_comp or key in true_indel_comp:
                            if key in true_snp_comp:
                                ...
                                if value[1] == true_snp_comp[key]:
                                    ...
                                    fp_snp_comp_file.write(true_snp_comp[key] + "\t" + "\t".join(value) + "\n")
                            ...
                            if key in true_snp_none:
                                ...
                                if value[1] == true_snp_none[key]:
                                    ...
                                    fp_snp_none_file.write(true_snp_none[key] + "\t" + "\t".join(value) + "\n")
                            ...
                            if len(value[1]) == 1:
                                ...
                                fp_snp_other_file.write("\t".join(value) + "\n")
                            else:
