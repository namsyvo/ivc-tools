...
for rl in read_lens:
    for err in seq_errs:
        for rn in read_nums:
            ...
            result_path = os.path.join(data_path, result_dir, "samtools_time_mem")
            if not os.path.exists(result_path):
                os.makedirs(result_path)
            bam_file = sam_path + "/" + read_fn + "_" + str(rl) + "." + str(err) + "." + str(rn) + ".bwa_sorted_RG_realign.bam "
            result_file = result_path + "/" + read_fn + "_" + str(rl) + "." + str(err) + "." + str(rn) + ".bwa_sorted_RG_realign.raw.vcf"
            cmd = "/usr/bin/time -v " + prog_path + "/samtools mpileup -uf " + ref_file + " " + bam_file + " 2>" + result_file + ".log" + " | " + prog_path + "/bcftools/bcftools view -vcg - > " + result_file
            print cmd
            os.system(cmd)
