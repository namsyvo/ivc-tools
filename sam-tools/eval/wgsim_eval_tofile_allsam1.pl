...
\n/) if (@ARGV == 0 && -t STDIN);
	...
	use English;
	my $dir = $path;
	foreach my $inputFile (glob("$path/*.sam")) {
		printf "%s\n", $inputFile;

		# -------------------------------------------------
		open (INPUTFILE, "$inputFile")  or die "can't read open '$inputFile': $OS_ERROR";
		...
		while (<INPUTFILE>) { # indicate the explicit inputfile - added by Nam S. Vo
			...
			#print FPFILE $line if (($flag&1) && !$is_correct && $q > 0); # write to file (added by Nam S. Vo)
		}
		...
		if (!defined($opts{a})) {
			...
			# printf EVALFILE ("%d\t%d\t%d\n", $c1[25], $c0[25]-$c1[25], $c0[25]) if ($cc0);  # write to file (added by Nam S. Vo)
		} else {
		...
		# print STDERR $fp, "\t", $mr, "\n";
		my $evalFile = $ARGV[0];
		open (EVALFILE, ">>$evalFile") or die "can't read open '$evalFile': $OS_ERROR";
		printf EVALFILE $inputFile;
		printf EVALFILE "\t";
		printf EVALFILE $fp;
		printf EVALFILE "\t";
		printf EVALFILE $mr;
