...
results_path = data_path + "/chr20_mutate1/results/3-24-2014-GATK"
#results_path = data_path + "/chr1_mutate1/results/GATK-BOTH"
...
for cvr in ['1x', '2x', '3x', '5x', '7x', '10x']:
	for seq_err in ['0.01', '0.02', '0.04']:
		called_snp_file = results_path + "/reads-100." + seq_err + "." + cvr + ".bwasw.vcf.txt"
		snp = {}
		...
		called_indel_fn = "indel-length/called_indel-length-gatk-human-" + seq_err + "." + cvr + ".txt"
		called_indel_file = open(called_indel_fn, "w")
		true_called_indel_fn = "indel-length/true_called_indel-length-gatk-human-" + seq_err + "." + cvr + ".txt"
