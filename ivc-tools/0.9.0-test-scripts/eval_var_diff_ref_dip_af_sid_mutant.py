...
data = open(os.path.join(data_dir, index_dir, "index_1.00", "GRCh37_chr1.fasta.mgf.idx")).readlines()
for line in data:
    if line[0] == '>':
...
known_var_file = os.path.join(ref_path, "dwgsim_reads_100.0.00015-0.0015.1246253.mutations.vcf")
unknown_var_file = os.path.join(ref_path, "dwgsim_reads_100.0.00015-0.0015.1246253.mutations.vcf")
...
with open(known_var_file) as f:
    for line in f.readlines():
        if line.strip() and line[0] != '#':
            ...
            pos, known_var, info = int(value[1]) - 1, value[3:5], value[7]
            phase = info.split(";")[1].split("=")[1]
            vtype = info.split(";")[2].split("=")[1]
            if phase == "1" or phase == "2":
                if vtype == "DELETE":
                    true_known_indel[pos] = [known_var[0], known_var[0] + "|" + known_var[1]]
                    KAKID += 1
                else:
                    if vtype == "SUBSTITUTE":
                        true_known_snp[pos] = [known_var[0], known_var[0] + "|" + known_var[1]]
                        KAKS += 1
                    else:
                        true_known_indel[pos] = [known_var[0], known_var[0] + "|" + known_var[1]]
                        KAKID += 1
                ...
                if vtype == "DELETE":
                    true_known_indel[pos] = [known_var[0], known_var[1] + "|" + known_var[1]]
                    KAKID += 1
                else:
                    if vtype == "SUBSTITUTE":
                        KAKS += 1
                        true_known_snp[pos] = [known_var[0], known_var[1] + "|" + known_var[1]]
                    else:
                        true_known_indel[pos] = [known_var[0], known_var[1] + "|" + known_var[1]]
                        KAKID += 1
'''
                    ...
                    if line.strip() and line[0] != '#':
                        ...
                        if pos in true_known_snp:
                            ...
                            if float(value[5]) >= confi_K_I:
                                var_call[pos] = value[3:5]
                        elif len(value[3]) == 1 and len(value[4]) == 3:
