'''
Evaluate variant call results, filter overlap indels
Usage: python eval_var_af_sid_mutant_diff_ref_filter_overlap_indel.py config_file confi cpu_num coverage_num result_dir_name
...
for para in ref_para[0:1]:
    ...
    result_file_path = result_path + "/" + read_fn + "_" + str(read_lens[0]) + "." + str(seq_errs[0]) + ".prec-rec-time-mem." + str(confi) + ".diff-pos-filtered.txt"
        ...
        for err in seq_errs:
            for rn in read_nums:
                for ms in max_snum[6:7]:
                            ...
                            if line.strip() and line[0] != '#':
                                ...
                                if float(value[5]) >= confi:
                                    ...
                                    if pos not in called_var:
                                        called_var[pos] = []
                                        called_var[pos].append(line)
                                    else:
                                        called_var[pos].append(line)
                    print "#called variants", len(called_var)

                    called_var_filtered = {}
                    called_var_filtered_to_file = {}
                    prev_key = -1
                    multi_aln = 0
                    for key in sorted(called_var):
                        val = called_var[key]
                        val_0 = val[0].strip().split()
                        if (len(val_0[3]) > 1 and prev_key + len(val_0[3]) > key) or (len(val_0[4]) > 1 and prev_key + len(val_0[4]) > key):
                                prev_val = called_var[prev_key]
                                prev_val_0 = prev_val[0].strip().split()
                                val_header = [x.split()[23] for x in val]
                                prev_val_header = [x.split()[23] for x in prev_val]
                                if len(set(val_header).intersection(set(prev_val_header))) == 0:
                                    print key
                                    multi_aln += 1
                                    if prev_key in called_var_filtered:
                                        del called_var_filtered[prev_key]
                                        del called_var_filtered_to_file[prev_key]
                                    prev_key = key
                                    continue
                                    '''
                                    if float(val_0[5]) > float(prev_val_0[5]):
                                        called_var_filtered[key] = val_0[3:5]
                                        called_var_filtered_to_file[key] = val
                                        if prev_key in called_var_filtered:
                                            del called_var_filtered[prev_key]
                                            del called_var_filtered_to_file[prev_key]
                                        prev_key = key
                                    ...
                                    '''
                        '''
                        if len(val_0[3]) > 1 or len(val_0[4]) > 1:
                            if int(val_0[12]) >= 2:
                                called_var_filtered[key] = val_0[3:5]
                        else:
                            called_var_filtered[key] = val_0[3:5]
                        ...
                        called_var_filtered_to_file[key] = val
                        prev_key = key
                    print "#multiple_aligned_pos", multi_aln
                    print "#filtered called variants", len(called_var_filtered)

                    called_var_filtered_file = open(os.path.join(result_path, prefix_fn + ".varcall." + str(cpu_num) + ".filtered.vcf"), "w")
                    for key in sorted(called_var_filtered_to_file):
                        for line in called_var_filtered_to_file[key]:
                            called_var_filtered_file.write(line)
                    called_var_filtered_file.close()
                    ...
                    FP_KAKID, FP_NID = 0, 0
                    FP_S, FP_ID = 0, 0
                    for var_pos, var in called_var_filtered.iteritems():
