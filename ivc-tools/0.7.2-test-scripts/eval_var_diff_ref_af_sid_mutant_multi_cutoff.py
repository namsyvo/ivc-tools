...
for para in ref_para[0:1]:
    ...
    result_file_path = result_path + "/" + read_fn + "_" + str(read_lens[0]) + "." + str(seq_errs[0]) + ".prec-rec-time-mem." + str(confi) + ".diff-pos-multi-cutoff.txt"
                    ...
                    with open(called_var_file) as f:
                        for line in f.readlines():
                            if line.strip() and line[0] != '#':
                                ...
                                if value[3] == value[4]:
                                    continue
                                called_var[int(value[1]) - 1] = value
                                if float(value[5]) != float(value[9]):
                                    diff_qual += 1
                                    #print float(value[5]), float(value[9])
                    print "# Called variants", len(called_var)
                    print "# Diff qual", diff_qual
                    print "Min Bayesian qual", min(called_var.iteritems(), key=lambda x: x[1][5])
                    print "Min Bayesian qual & mapping qual", min(called_var.iteritems(), key=lambda x: x[1][9])

                    for cutoff_val in [x * 0.01 for x in range(114, -1, -1)]:
                        cutoff_called_var = {}
                        for pos, val in called_var.iteritems():
                            if float(val[2]) >= cutoff_val:
                                cutoff_called_var[pos] = val[:2]
                        #print "# cutoff called variants", len(cutoff_called_var)
                        ...
                        FP_KAKID, FP_NID = 0, 0
                        FP_S, FP_ID = 0, 0
                        for var_pos, var in cutoff_called_var.iteritems():
                        ...
                        nums_header = ["Alg", "cov", "qual", "TP-S", "FP-S", "FP-S-N", "TP-S-U", "FP-S-U", "TP-S-K", "FP-S-K", \
                                    "TP-I", "FP-I", "FP-I-N", "TP-I-U", "FP-I-U", "TP-I-K", "FP-I-K"]
                        '''
                        result_file.write("\t".join([prog_version, "%.0f" % (2.0*int(rn)*int(rl)/ref_len), str(cutoff_val)]) + "\t")
