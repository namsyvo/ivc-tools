...
// Fall 2013 - Quang
// Nam S. Vo, Sep 2014
// Modified by Nam S. Vo, Jan 2015: generate mutations based on random allele frequency
///////////////////////////////////////////////////////////////////////////////////////
...
func main(){
	...
	for _, prob := range NEW_VAR_PROB {
		...
		mutate, known_variants, new_snps, new_indels, known_var_prob := GenerateMutate(seq, SNP_Prof, prob)
		fmt.Println("Saving everything...")
		SaveMutations(path.Join(os.Args[3], "new-mutate-" + strconv.FormatFloat(prob, 'f', 5, 32)), header, mutate, known_variants, new_snps, new_indels, known_var_prob)
	}
}
...
//--------------------------------------------------------------------------------------------------
func GenerateMutate(seq []byte, SNP_Prof map[int]SNPProfile, new_var_prob float64) ([]byte, map[int][]byte, map[int]byte, map[int][]byte, map[int]float64) {
	mutate := make([]byte, 0)
	known_variants := make(map[int][]byte) //known snps and indels
	known_var_prob := make(map[int]float64) //allele freq of snps 
	...
	for snp_idx = 1; snp_idx < len(snp_pos); snp_idx++ {
		...
		if rand.Float32() < float32(new_var_prob) {
			...
			//Assumption: there is only biallelic SNPs
			af = rand.Float64()
			known_var_prob[pos] = af
	...
	return mutate, known_variants, new_snps, new_indels, known_var_prob
}

//--------------------------------------------------------------------------------------------------
// Save mutated genome, mutation profiles to file
//--------------------------------------------------------------------------------------------------
func SaveMutations(dir_name string, header, mutate []byte, known_variants map[int][]byte, new_snps map[int]byte, new_indels map[int][]byte, known_var_prob map[int]float64) {
	...
	for _, pos := range known_variant_pos {
		...
		_, err := kv_file.WriteString(snp_pos + "\t" + string(known_variants[pos]) + "\t" + strconv.FormatFloat(known_var_prob[pos], 'f', 5, 32) + "\n");
		if err != nil {
            fmt.Println(err)
            break
        }
