...
sam_path = data_path + "/chr1_mutate1/results/all-sams"
...
for cvr in ['1x', '2x', '3x', '5x', '7x', '10x']:
	for seq_err in ['0.01', '0.02', '0.04']:
		cmd = script_path + "/gatk-callsnp.sh " + ref_path + "/chr1.fasta " \
		+ sam_path + "/reads-100." + seq_err + "." + cvr + ".bwasw_sorted_RG.bam " \
		+ ref_path + "/vcf_chr_1-diffcontigname.vcf " \
		+ results_path + "/reads-100." + seq_err + "." + cvr + ".bwasw.vcf " + tool_path + " &"
		os.system(cmd)
